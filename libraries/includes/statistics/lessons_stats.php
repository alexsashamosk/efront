...
if (isset($_GET['excel']) && $_GET['excel'] == 'lesson') {
    ...
    if (isset($_GET['group_filter']) && $_GET['group_filter']) {
        try {
            ...
            $groupnumber_course = str_replace(" ", "_" , $group -> group['number_course']);
        } catch (Exception $e) {
            $groupname = false;
    ...
    //basic info
    if ($groupname || $branchName || $groupnumber_course) {
        $celltitle = "";
        if ($groupname) {
            $celltitle .= _BASICINFO . " " . _FORGROUP . ": ". $groupname . " ";
        }
        if ($groupnumber_course) {
            $celltitle .= _BASICINFO . " " . _FORGROUP . ": ". $groupnumber_course . " ";
        }
        if ($branchName) {
            if ($celltitle != "") {
